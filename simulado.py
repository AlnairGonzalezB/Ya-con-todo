import random as rnd
import numpy as np

## Esta funcion regresa a los hijos de un estado , segun el problema
def getHijos(edo ,proble):
    return proble[edo] 

## Esta fucion regresa al mejor estado en la lista
def siguiente(lista ,hs): 
    hmejor =1000
    mejor=' '
    donde=0
    indi=0
    for edo in lista:
        if hs[edo]<hmejor:
            mejor=edo
            hmejor=hs[edo]
            donde=indi
        indi=indi+1
    lista =[]
    return lista ,mejor ,hmejor

## Esto implementa el templado simulado
def temSim(actual ,k,T,lista ,proble ,hs):
    hactual=hs[actual]
    if k>35:
        soln=actual
    else:
        lista=getHijos(actual ,proble)
        lista ,mejor ,hmejor=siguiente(lista ,hs)
        if hmejor >hactual:
            p=rnd.uniform (0,1)
            razon=np.exp((hactual -hmejor)/T)
            if p<razon:
                nuevo=mejor
            else:
                nuevo=actual
        else:
            nuevo=mejor
        soln=temSim(nuevo ,k+1 ,0.75*T,lista ,proble ,hs)
    return soln
## Esta es la funcion preincipal del programa
def main():
## El grafo del problema
    proble={'A':['B','C','D','E','F'],'B':['A','C'],'C':['A','B','D','G','H'],'D':['A','C','E','I'],'E':['A','D','F','J','K'],'F':['A','E'],'G':['C','H'],'H':['G','C','I','L'],'I':['D','J','N','M','L','H'],'J':['K','E','Ñ','I'],'K':['J','E'],'L':['M','I','H','O'],'M':['I','N','O','L'],'N':['I','M','Ñ','O'],'Ñ':['J','N'],'O':['L','M','N']}
    hs={'A':10,'B':11,'C':10.5,'D':10.25,'E':8,'F':12,'G':9,'H':7.5,'I':10,'J':12.25,'K':12,'L':9,'M':7,'N':10,'Ñ':11,'O':8.5} 
    soln=temSim('A',0,30,[],proble ,hs)
    print('SOLUCION ENCONTRADA:'+soln)

## Este es el punto de entrada al programa
if __name__ =='__main__':
    main()
